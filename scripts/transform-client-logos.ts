...
function assertLiveTarget(target: MigrationDatabaseTarget): void {
  ...
  assert.equal(target.database, 'litigation', 'Task 2.11 live database must be litigation');
}
    ...
    async (db) => {
      ...
      try {
        ...
        assert.deepEqual(
          plan,
          preview,
          'source or mapping changed between dry plan and transaction',
        );
