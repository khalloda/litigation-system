...
import { migrationDatabaseTarget, withApprovedMigrationClient } from './lib/migration-principal';
import {
...
async function main(): Promise<void> {
  ...
  const targetDatabase = migrationDatabaseTarget();
  if (!['localhost', '127.0.0.1'].includes(targetDatabase.hostname) || targetDatabase.port !== 5433)
