...
out.push(`    IF n <> ${tableCount} THEN`);
out.push(
  `        RAISE EXCEPTION 'staging: % primary keys, expected ${tableCount}', n;`,
);
out.push('    END IF;');
out.push('');
out.push(
...
console.log(`  tables       : ${tableCount}  (${manifestTableNames.length} extracted + ${complexTables.length} complex)`);
