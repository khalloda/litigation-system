...
async function gate4MigrationEvidence(db: Client, checkpoint: 60 | 61 = 61) {
  ...
  return reconcileGate4Migrations(migrationHistory, repository);
}
    ...
    if (
      (
        await db.query<{ present: boolean }>(
          `SELECT to_regclass('public.audit_events') IS NOT NULL present`,
        )
      ).rows[0]?.present
    ) {
      if (
      ...
      await db.query(`
        DROP TABLE public.audit_event_checkpoints;
        ...
        DROP FUNCTION public.audit_set_event_context(uuid,uuid,uuid,inet,text,text);
        DROP FUNCTION public.audit_safe_flat_object(jsonb);
        DROP FUNCTION public.audit_bound_json_value(jsonb,integer,text);
...
async function proveHistoricalUpgrade(admin: Client, source: URL): Promise<void> {
  ...
  try {
    ...
    await admin.query(`CREATE DATABASE ${identifier(fixtureName)} TEMPLATE template0`);
    created = true;
    const fixture = new Client({ connectionString: fixtureUrl.toString() });
    await fixture.connect();
    try {
      await prepareHistoricalAuditFixture(fixture, fixtureUrl);
    ...
    try {
      ...
      const evidence = await gate4MigrationEvidence(upgraded, 60);
