 ...
 * it were a decision.
 *
 * Because the seed is GENERATED, fixing it is one change to one source file
 * and a regeneration. Had these 150 values been typed into a migration by
 * hand, the correction would be a hunt through them.
 *
...
out.push('--');
out.push('--  PROVISIONAL: hearing_action, matter_destination and client_branch were');
out.push('--  marked "already clean" without inspection and are being re-analysed.');
out.push('--  At least محكمة / محكمه / مجكمة are one word typed three ways.');
out.push('--  Task 1.6 (Arabic search) is blocked until corrected values arrive.');
out.push('--');
