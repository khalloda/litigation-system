...
export async function task210aProtectedState(db: ClientBase): Promise<string> {
  ...
  const result = await db.query<{ digest: string }>(
    `SELECT encode(sha256(convert_to($1||E'\\n'||coalesce(string_agg(payload,E'\\n' ORDER BY kind,identity),''),'UTF8')),'hex') digest
       FROM (
         SELECT 'F' kind,id::text identity,(to_jsonb(f)||'{"created_by":null,"updated_by":null}'::jsonb)::text payload FROM fee_letters f
         UNION ALL SELECT 'FLM',id::text,(to_jsonb(l)||'{"created_by":null,"updated_by":null}'::jsonb)::text FROM fee_letter_matters l
         UNION ALL SELECT 'MFR',id::text,(to_jsonb(r)||'{"created_by":null,"updated_by":null}'::jsonb)::text FROM matter_fee_letter_references r
