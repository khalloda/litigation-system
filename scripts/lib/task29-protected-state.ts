  ...
  const result = await db.query<{ digest: string }>(`
    SELECT encode(sha256(convert_to(string_agg(payload, E'\\n' ORDER BY kind, identity), 'UTF8')), 'hex') digest
      FROM (
        ...
        UNION ALL SELECT 'C',id::text,(to_jsonb(c)||'{"created_by":null,"updated_by":null}'::jsonb)::text FROM clients c
        UNION ALL SELECT 'O',id::text,(to_jsonb(c)||'{"created_by":null,"updated_by":null}'::jsonb)::text FROM contacts c
        UNION ALL SELECT 'M',id::text,(to_jsonb(m)||'{"created_by":null,"updated_by":null}'::jsonb)::text FROM matters m
        UNION ALL SELECT 'QM',id::text,to_jsonb(q)::text FROM quarantine.matter_transform q
        UNION ALL SELECT 'ML',id::text,(to_jsonb(l)||'{"created_by":null,"updated_by":null}'::jsonb)::text FROM matter_lawyers l
        UNION ALL SELECT 'MP',id::text,(to_jsonb(p)||'{"created_by":null,"updated_by":null}'::jsonb)::text FROM matter_parties p
        UNION ALL SELECT 'MR',id::text,(to_jsonb(r)||'{"created_by":null,"updated_by":null}'::jsonb)::text FROM matter_party_roles r
        ...
        UNION ALL SELECT 'H',id::text,(to_jsonb(h)||'{"created_by":null,"updated_by":null}'::jsonb)::text FROM hearings h
        UNION ALL SELECT 'HA',id::text,(to_jsonb(a)||'{"created_by":null,"updated_by":null}'::jsonb)::text FROM hearing_attendees a
