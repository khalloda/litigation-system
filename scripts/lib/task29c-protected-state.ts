...
export async function task29cProtectedState(db: ClientBase): Promise<string> {
  ...
  const result = await db.query<{ digest: string }>(
    ...
    SELECT 'P' kind,id::text identity,to_jsonb(p)::text payload FROM powers_of_attorney p
    UNION ALL SELECT 'PL',id::text,to_jsonb(l)::text FROM power_of_attorney_lawyers l
