...
- Exact next return point: Task 3.4 — User management, approved but not started,
  only after independent acceptance and owner-authorized push of this commit.
...
- atomic typed semantic contracts for future archive, restore, account
  lifecycle, role change, report, export and download operations;
...
| `audit_event_fields` | 262 explicit field rules and per-field text bounds; no complete-row serialization |
...
The 262-field allowlist excludes audit-maintenance columns, normalisation
shadows, every `legacy_*`/raw source payload, JSON/structured operational
payload and binary data. `user_accounts.password_hash` has the only explicit
`redacted` rule: its change creates an event, but its value never does.
Relationship events contain their IDs and relationship facts rather than
duplicating parent records.
...
removals; ordinary record tables keep D25's no-delete boundary. Existing
Task 3.3A context and event context are transaction-local. The row write and
event append therefore commit together, and a corrupted/missing mandatory event
context rolls both back. The future semantic wrapper runs the protected
callback and its event in one transaction and never accepts a result without a
...
an older protected digest merely because the new schema exists. The current
non-writing Gate 4 report digest is separately
`ef4be031694a8d8c9458925d8f3c337559d77c5805e379a03069d5d1b7666477`;
it changes from the retained Task 3.3A checkpoint only because the proven
repository inventory now includes migration 57. All six business report
datasets, source evidence and frozen Gate 4 evidence remain exact.
...
14. All nine later semantic actions commit their protected callback and event
    together; forced event failure rolls the callback back.
...
Historical-live provenance remains 51 required migrations + six later
...
The exact return point is **independent review and owner-authorized push of this
local Task 3.3B commit**. After acceptance, resume at **Task 3.4 — User
