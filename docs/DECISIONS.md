...
## D62 — Bounded administrative-work and task-step editing

**Adopted by Khaled Helmy on 14 September 2026 in his direct implementation authorization**, accepting the Phase 1 publication PASS at `fbf47d7fc0e5a1405a8798838243289d2d31898f` and the supplied Phase 2 proposal and prompt. This is a new administrative-work contract; it does not extend D60 implicitly. D1–D61, including D59, remain unchanged.

1. Administrator, Litigation Assistant and Paralegal may create/update administrative works and steps. Lawyer views only. Enforce existing administrativeWorks permissions at page, action, service and committing database gateway. Obtain account, actor, role, session version and expiry from validated server context; recheck current enabled account, active internal login identity, password/reset state and expiry before any write or receipt reuse.
2. Editable task fields are requiredWork, assignedToPersonId, taskCreatedDate, executionDate, result, previousDecision, lastFollowup, deadline, courtId, circuit, destinationId, status and alert. Status and alert remain ordinary text facts, with no status machine, completion inference or notification. lastFollowup remains complete multiline text. PostgreSQL createdAt is not the task business date.
3. Editable step fields are actionDate, performedByPersonId, result and report. nextAppointment stays outside UI and mutation payload; preserve existing values and use NULL for native steps.
4. New tasks need meaningful required-work text; new steps need meaningful result or report. Existing missing descriptions and incomplete steps remain valid on unrelated edits and true no-ops. An explicit changed description or result/report pair must leave the required content meaningful. Checking completeness never trims or rewrites unchanged stored text.
5. A new task explicitly chooses an existing unarchived matter or unassigned NULL. No implicit client/matter selection. Existing task matter links, including NULL, are immutable. Every new step belongs to the exact existing task; that parent remains fixed. No detached new steps, moves, cascading relinks or quarantine release.
6. All Phase 1 reads remain available beneath archived matters and clients. Creation beneath a matter and edits to its task/steps require the matter unarchived at commit; Administrator must restore it first too. Lock/recheck parent state against concurrent archive. A task with NULL matter has no restoration prerequisite. An archived client does not block its otherwise unarchived matter. This phase adds no task/step archive flag, archive/restore UI or deletion.
7. New assignments select active internal staff by ID, independently of login eligibility. New court/destination values select active existing lookup IDs. Recheck and lock eligibility at commit. Preserve current inactive references and exact labels on unrelated saves; allow deliberate nullable clearing/replacement. Do not create or match people/lookups by display name. The forms expose every eligible row, with no arbitrary result cap; the earlier read-filter behavior is unchanged.
8. Dates are optional date-only values, without automatic today, inferred ordering or extraction from follow-up text. New tasks explicitly enter their business creation date or choose not recorded. Omitted fields mean preserve on update and NULL/default on creation; explicit JSON null clears a nullable field; empty JSON text remains empty text where completeness permits. The UI maps a deliberately emptied optional control to NULL. Unchanged Unicode, spacing and line endings remain exact; a changed textarea uses its entered browser line endings. No trimming or silent truncation occurs.
9. Technical changed-value limits are 160 UTF-16 code units for status (Arabic and ordinary Latin characters use one; supplementary characters use two), to remain addressable by the accepted status filter, and 10,000 Unicode code points for other text. The serialized request is limited to 200,000 UTF-8 bytes at server and database boundaries; each boundary validates its JSON representation. These limits exceed measured source maxima (requiredWork 546; task result 272; previousDecision 395; lastFollowup 233; circuit 49; status 7; alert 5; step result 245; report 5) and impose no historical cleanup or legal meaning. Valid calendar dates range from year 0001 through 9999. Unknown, excluded, source, actor, identity and parent-update fields are rejected.
10. Preserve all imported IDs, provenance/raw fields, source hashes/payloads, reviewed mappings and sourceOrdinal independently of current edits. Native records receive application IDs and NULL Access provenance. Imported steps retain recorded sourceOrdinal order with ID tie-break; native steps append afterward using immutable separate currentOrder, allocated under the task lock. Changing date/content never reorders a step. No physical deletion or bulk reordering.
11. A task and its steps share a stable aggregate version. Any changed task or step save advances it once, with attributed append-only complete before/after history, submission receipt and audit facts in the same transaction. Stale forms refuse rather than overwrite; preserve entered text and show Arabic recovery guidance. Bind every step save to exact task and aggregate version. A true no-op writes no business, audit, history, receipt or sequence state.
12. An exact same-actor/same-payload committed retry returns the recorded result, after fresh authority checks, without replaying a write. Changed payload/action or another actor's token is refused. A prior receipt may acknowledge an earlier result after a later parent change; the detail shows fresh current state. No-op tokens create no receipt. Genuine failed inserts may consume PostgreSQL ID reservations; measure and disclose them, never rewind. Preserve shared audit counter/actor safeguards.

Practical consequence: a Paralegal can record work and progress under an active matter, while a Lawyer can read it. If another user changes the task or a step while a form is open, the older form cannot overwrite that change. An incorrect existing parent link cannot be moved through these forms; an archived matter needs Administrator restoration first. These are the adopted trade-offs. No new licence, dependency, hosted service, export, attachment, notification or billing feature is introduced. Migration 68 and the candidate remain unactivated on the actual database; independent implementation review and separate owner acceptance/activation/publication decisions follow.


## D63 — Administrative work and step lifecycle

**Explicitly adopted by Khaled Helmy on 15 September 2026**, in the direct Phase 3 implementation instruction accepting the combined Phase 2 PASS at `f05787471812c8fb2bd03d31bc44614dad81a8d2`, including its authenticated supplement. Candidate implementation/testing is authorized only on positively owned disposable copies; actual migration 69, activation and publication require later review and authorization.

1. Permission and meaning. Only Administrator may archive or restore an administrative work or an individual task step. Administrator, Litigation Assistant and Paralegal retain D62 create/update where the record and required parents are unarchived; Lawyer retains reads only. All four roles may read archived records. Archive means reversible removal from ordinary operational use, never deletion, completion, cancellation, a new status, a new legal outcome or a change to dates. Enforce current server-derived account/session/role/actor at page, action, service and committing database gateway.

2. Independent states and initialization. A work and each step have separate archive states. All existing and newly created works/steps start unarchived; do not infer archival from old dates, status, missing fields, inactive references or an archived parent. Archiving/restoring a work changes no step's archive flag or business values. Archiving/restoring a step changes no sibling/work/matter/client archive flag. Restoring a work must not restore a separately archived step. Parent matter/client archive transitions retain their existing non-cascading behavior. No hard delete, purge, relink or order rewrite is introduced.

3. Parent prerequisites. Before a NEW work archive/restore or ordinary work edit, its associated matter must be unarchived. Before step creation, step edit or a NEW step archive/restore, the owning work and its associated matter must be unarchived. This applies to Administrator too. A work with NULL matter has no matter-restoration prerequisite; an archived client does not block its otherwise unarchived matter. Archived works remain readable with all step history accessible. An archived step remains readable but cannot be edited until restored. An active step inside an archived work is blocked by its parent, not falsely described as independently archived. The work may still be edited when it contains independently archived steps; those steps remain unchanged. Unexpected detached current steps are preserved and reported, never attached or released from quarantine by this phase.

4. Operational visibility and context. Work lists default to unarchived works with explicit current/archived/all archive filtering, independent of q, matter, client, person, business status, page and validated fromMatter context. A work is not hidden merely because its matter/client is archived. Step listings default to unarchived steps with an independent current/archived/all step filter, including on archived work details. Show truthful total, current and archived step counts and an explicit way to view the complete history. Direct archived reads remain available and clearly labelled. Every list/detail/confirmation/edit/Cancel/back link preserves the applicable work filter, step filter, search, parent-return and pagination context. After a transition, clamp an empty/out-of-range page safely and explain the result. Do not let a list filter become authorization or a direct-read exclusion.

5. Historical content and order. Preserve all business/raw/source values, immutable IDs/parent associations, imported sourceOrdinal, native currentOrder, mappings, imported payloads/hashes, inactive/missing references, dates, NULL versus empty text and multiline text. The existing current-versus-original-source display correction remains intact: a cleared current reference shows غير مسجل and retained original text stays separately labelled. nextAppointment remains hidden and unchanged. Archiving creates a visibility gap, not renumbering; restoration returns the step to its unchanged original position. New native steps append after all earlier native orders, including archived ones, and never reuse an archived step's order. No people/lookup reactivation, notification, financial, hearing, document, matter/client or quarantine mutation is introduced.

6. Reports and linked counts. Existing report/service inclusion, criteria, historical counts and financial totals remain independent of work, step, matter and client archive state. Operational defaults must not leak into report sources. Current linked work/step navigation offers an explicit route to archived/all records. Preserve the existing meaning of total step/history counts; distinguish a filtered visible count instead of silently relabelling a smaller number as the historical total. Confirmation/help explains why archived items can remain in reports. New report/export screens and audit-history UI are outside this phase.

7. Deliberate confirmation. Each work/step archive or restore requires an Arabic confirmation naming the exact subject with its work/matter context and available identifying date/text. Show useful current/archived/total step or sibling counts and any other actual schema relationships whose retention needs explanation; distinguish counts from affected writes. Recheck authorization, subject/parent IDs, current state, aggregate version, parent eligibility and the confirmation's count/snapshot facts at commit. A changed confirmation refuses safely and refreshes for deliberate reconfirmation. Do not require empty steps, completed work, paid bills, new mandatory reasons or cleanup of old data. Cancel and confirmation reads create no business/audit/history/receipt/sequence state.

8. Shared version, trusted history and audit. Keep the existing work-plus-steps aggregate version. A changed transition advances it exactly once and records complete attributed before/after history, owned submission receipt and the correct archive/restore audit fact for the actual subject in one transaction. A step transition may require the established parent-version bookkeeping/audit; do not misreport it as a user request to archive the whole work or duplicate the subject's archive event. Ordinary writes cannot smuggle archive fields. New lifecycle writes cannot edit business/provenance/parent/order fields. Preserve complete prior import, boundary, change, submission and audit evidence byte-for-byte. Adapt historical interpretation at an explicit version/schema boundary without stripping archive state from new history or rewriting old JSON.

9. No-op, retry and concurrency. Fresh authority checks precede every receipt reuse. A valid unchanged requested state with the current version and current confirmation facts is a true no-op, with no business/audit/history/submission/sequence change. Stale versions or confirmation facts are refused. An exact same-actor/same-action/same-payload committed retry may acknowledge its original result after fresh authentication even after later archive/restore or parent change; it must not replay a write, override current state or present an old receipt as today's state. Changed payload/action/subject/parent or another actor's token is refused. Scope submission IDs across editing and lifecycle gateways so a token cannot cross operations. Lock/recheck parents and the aggregate consistently against concurrent matter/work/step archive, create, edit and restore. Stale open editors retain entered text and cannot save over a later archive or silently restore a record. Never rewind a sequence; measure/disclose genuine failed-insert reservations separately from true no-ops.

10. Upgrade and permanent protection. Add only a forward migration after frozen migrations 1–68, expected migration 69, for this contract. Preserve all old-column projections, original sequences and old metadata/history at upgrade; only the reviewed new schema/boundary/default archive-state/audit-classification/migration-ledger delta is permitted. Existing edited and native D62 aggregates and historical receipts must remain valid. New invariant checks cover complete mappings/history/authorization and current state, not just row counts. Existing credentials, D59, old governance/decisions and unrelated business rules stay fixed. The candidate is tested on owned isolated copies and delivered for independent review before actual migration, accepted-runtime replacement or publication.


## D64 — Powers-of-attorney editing and recoverable archive/restore

Prepared 15 September 2026 for Khaled Helmy's combined Task 4.5 mandate. This is a proposed POA-specific contract until adopted by the accompanying direct implementation instruction; it is not a retrospective interpretation of D25. On adoption, append this exact contract to `docs/DECISIONS.md`, preserving D1–D63. Candidate implementation and testing are authorized on disposable copies. Actual database migration, activation, publication and final task acceptance follow independent review.

1. **Roles and scope.** All four existing roles view POAs, including explicitly requested archived records. Administrator and Litigation Assistant create and edit POAs and their current lawyer selections. Only Administrator archives/restores. Lawyer and Paralegal view only. Keep the existing `powersOfAttorney` permission matrix. Enforce current server-derived identity, enabled account, active login-eligible identity, session version/expiry/reset state, actor and permission at each protected page, action, service and committing database gateway, including receipt reuse. No fifth role, deletion permission, billing, document-register, movement, report/export or audit-history UI is added.

2. **Field meanings.** D29 remains binding. `الصفة` is free-text principal capacity, distinct from D7 party roles. Never use the abandoned `صفة الموكل بالتوكيل` duplicate as an application field, fallback, search term or filter. `رقم التوكيل`, `حرف` and `السنة` are separate text components of the business reference, shown together without inventing missing parts. `مسلسل` is text; Latin A/B/C are valid. Do not require reference uniqueness or numeric-only number/year/serial values without a separate business decision. Internal PostgreSQL IDs are technical identities; all imported `legacy_id` values remain NULL. Do not fabricate an Access numeric ID.

3. **Editable current values.** The writable allowlist is current client ID, serial text, principal name, maintained capacity, POA number, letter, year, issuing authority, issue date, copies count, notes, report-inclusion setting, and explicit current lawyer selections by person ID. New POAs require a nonblank principal name; other business values may be unrecorded. Existing incomplete imported fields do not require cleanup on unrelated saves. Optional client clearing/replacement is deliberate and audited, never inferred from text. Omitted update fields preserve current values; explicit NULL clears a nullable field; an intentionally emptied optional UI control sends NULL. Preserve untouched Unicode, spacing, line endings, NULL versus empty text and full multiline content. Do not silently trim or truncate. New records have application IDs and NULL Access identity/provenance; never manufacture import metadata.

4. **Copies and report flag.** `عدد النسخ` is the current number of copies in the safe: zero means none available, NULL means unknown, and values greater than one are ordinary valid data. Accept nonnegative whole numbers within the existing PostgreSQL integer representation, with no arbitrary business cap. A manual count edit is an attributed correction, not an inferred custody transaction. `جرد` controls POA-list-report inclusion; it is an editable boolean with an explicit unrecorded state where NULL is supported. Report-hidden records remain visible in the application. Do not infer count, availability, archive, custodian, revocation or legal validity from each other or from notes. Do not implement check-out, check-in or movement cards; those remain Task 6.7.

5. **Dates and limits.** Issue date is optional, date-only, a valid calendar date in years 0001–9999; no automatic today, expiry or date-order rule. Use measured source maxima and the established mutation pattern to impose generous technical changed-value and request-size limits, validated at server and database boundaries. Use 10,000 Unicode code points per changed text field and 200,000 UTF-8 bytes per serialized request unless measured legitimate data requires a larger documented technical limit. These are transport protections, not legal rules; preserve unchanged historical values. Reject unknown/excluded fields, forged identity/provenance/audit fields, nonintegral or negative counts, invalid dates, duplicate selected IDs and malformed payloads.

6. **Current lawyer assignments and historical evidence.** Seed current membership from exactly the 87 reviewed imported links, with their recorded source ordering, never from the 717 unresolved/partial/excluded relationship-evidence rows. Preserve all original reviewed link rows, rule/member provenance, raw cells, source keys, hashes, payloads and quarantine evidence independently and unchanged. Maintain current assignments separately from that immutable import evidence, using the project's retained-membership/history pattern. Explicit removal retires a current membership; it never deletes the original link or person. Explicit re-addition reactivates or appends current membership deterministically with recorded history. No duplicate active person membership and no bulk reordering. Preserve imported order; append new memberships in entered order with stable tie-breaking, without reusing retired positions. Restore of a POA does not reactivate a separately removed membership. New selection accepts active existing people by ID, including external counsel; do not limit it to internal staff or login accounts. Preserve existing inactive selections on unrelated saves, but do not newly select/reactivate an inactive person. No person creation, name matching, alias creation, rule amendment or quarantine resolution is implicit in a POA edit. An operator's explicit current assignment does not rewrite the historical migration decision.

7. **Current and source display.** List/detail/editor distinguish current client and current lawyer assignments from retained imported client/name text and reviewed original links. After clearing a current reference, show `غير مسجل` for that current field; never substitute old imported text into it. Source text remains separately labelled and complete, including partially unresolved compounds. Do not label a POA with no reviewed/current links as having no lawyer in reality. All historically linked inactive/external people remain readable; do not link an external person to a staff-only route that rejects them. The full source payload and abandoned duplicate remain preserved for verification, without creating a raw JSON or duplicate-field business UI.

8. **Parent client and state.** A POA with NULL current client is valid. Creation and replacement client selection use existing unarchived clients. Before an ordinary edit or a NEW archive/restore operation on a POA already linked to a client, that current client must be unarchived; Administrator must restore it first too. For relinking, lock and recheck both the old and selected client consistently; clearing may not bypass an archived current parent. Historical POA reads remain available beneath an archived client. POA archive/restore never changes client, person, current membership retirement, business values, copies or report flag; client archive/restore never cascades to POA state. No relationship to a matter is invented from the client link.

9. **Archive meaning and visibility.** Archive is recoverable removal from ordinary operational use, not revocation, cancellation, expiry, invalidity, disposal or a custody movement. All existing and native POAs start unarchived; do not infer archive state from source data, missing references, copies, report flag or parent state. Lists default to unarchived POAs with explicit archived/all filters, independently of all other filters. All roles can directly read archived details with clear state labels. Archived business values and memberships are read-only until Administrator restores the POA and its parent permits editing. Retain historical counts and the independent report-inclusion meaning; do not make operational archive defaults an implicit future report exclusion.

10. **Confirmation and navigation.** Archive/restore requires a deliberate Arabic confirmation identifying the exact POA by technical ID, available principal/business reference and client, and showing accurate current/retired/original relationship counts as appropriate. Explain retention and that archive is not legal revocation and does not change copies or report inclusion. Recheck subject, current authority, aggregate version, parent eligibility and displayed confirmation facts at commit. Changed facts require refresh and reconfirmation. Do not require a particular date, zero copies, no lawyers, a new mandatory reason or historical cleanup. List, detail, create/edit and confirmation/Cancel/back preserve validated local search/filter/pagination context. Confirmation reads, Cancel, Escape and ordinary navigation cause no business/audit/history/submission/sequence writes.

11. **Atomic saves, conflicts and retries.** One POA and its current memberships share an aggregate version. Each real edit, membership change or archive/restore advances it exactly once, with complete attributed before/after history, owned submission receipt and correct audit facts in one transaction. Ordinary editing cannot set lifecycle fields; lifecycle cannot change business/source/membership fields. Prevent runtime direct table/sequence writes and untrusted actor/context forgery. Use deterministic locking across aggregate, applicable client, selected people and audit/session state. Stale forms refuse without overwriting, preserving entered text with Arabic recovery guidance; an editor opened before archive cannot silently save or restore. A valid current-version identical save/state is a true no-op with no business/audit/history/receipt/sequence change. Fresh authentication precedes receipt lookup/reuse. Exact same-actor/same-action/same-payload committed retry may acknowledge its original result without replaying writes, even after later changes; display freshly read current state. Reject altered action, subject, payload or actor/token reuse across gateways. Genuine failed insert ID reservations are measured and disclosed, never rewound or confused with no-ops.

12. **Upgrade and permanent proof.** Freeze migrations 1–69 and all prior decisions/governance. Add a forward candidate migration, expected migration 70, for the combined POA boundary, current membership state, aggregate version/history/receipts and archive state. At upgrade preserve every old-column value, reviewed mapping, original sequence state, source/quarantine/receipt/history/audit record and unrelated database definition except an explicitly reviewed POA boundary delta. Keep an immutable, complete import/upgrade boundary and enforce current state against that baseline plus complete authorized history; do not make legal editing possible by disabling old reconciliation checks or merely counting rows. Add durable invariant and corruption checks for values, memberships/order, authorization, audit, versions, receipts, source preservation and current-state replay. Prove both an upgrade from the accepted migration-69 copy and the established canonical fixture path. Actual migration 70, owner application replacement and publication are outside this candidate run.

Practical trade-offs: this combined candidate enables data entry and recovery together, with a larger implementation and review than a read-only phase. A POA beneath an archived client needs that client restored before modification. Current lawyer assignments can change explicitly while historical text and migration decisions remain intact. Copies are manually maintained until the separately scoped movement workflow is built. There is no new licence or service requirement; usage and test time will be higher than for the proposed read-only run. These are explicit POA-specific choices adopted through the direct instruction, not rules silently inherited from another module.


## D65 — Document register editing and recoverable archive/restore

Prepared 16 September 2026 for Khaled Helmy's combined Tasks 4.6–4.7 implementation mandate. These are recommended module-specific rules, adopted when the accompanying direct owner instruction is sent. Append this contract and D66 once, after D64, preserving all earlier decisions. They authorize a complete implementation candidate and isolated proof; actual owner migration, activation, publication and final acceptance follow independent review.

1. **Purpose and roles.** Documents are the paper-document register, not an electronic file store. All four roles can read current and explicitly requested archived documents. Administrator and Litigation Assistant can create/edit; Administrator alone can archive/restore. Lawyer and Paralegal remain read-only. Preserve the existing `documents` permission decisions and enforce fresh identity/authorization at pages, actions, services and committing database gateways, including retries. M-Files remains the electronic-file system; no upload, file storage, M-Files API integration, physical check-in/check-out, movement transactions or Task 6.7 reports are introduced.

2. **Current editable values.** Allow description, document date, page count, deposit date, current client, current matter, current responsible person, storage location, movement-card reference text, notes and optional M-Files reference. A new document requires a nonblank description; the other business fields may be unrecorded. Existing incomplete imported records remain editable without forced cleanup of unrelated fields. `movement_card` remains descriptive/reference text, not a custody ledger or a generated movement card. Adding an M-Files reference is an explicit current edit; it never creates a legacy source value.

3. **Source evidence and identities.** Preserve the 407 imported records' Access IDs, source keys/hashes/payloads, original names/references/page-count text, all 372 field-evidence rows and source reconciliation in an immutable boundary. All imported M-Files source values remain NULL in that boundary. Current M-Files values may subsequently be entered and audited without falsifying the import. New application records have new technical IDs and NULL Access/provenance fields. Show current values separately from clearly labelled original-source text; NULL current values must not fall back to old text after clearing or replacement. Compound counts such as `21 + CD` remain source evidence and must never become an inferred numeric count.

4. **Relationship changes.** Current client, matter and responsible-person links may be explicitly added, cleared or replaced using stable IDs. No fuzzy name matching, automatic resolution of quarantined references, automatic client reassignment from a matter, or inferred person creation. Preserve existing mismatched, unresolved, inactive and missing-reference history; flag relevant context without silently repairing it. New or replacement person choices come from existing active eligible lawyer/staff identities under the project's roster rules; an unchanged historical inactive person can be retained. New client/matter selections exclude archived records. No new same-client restriction is imposed on historical associations.

5. **Parent eligibility.** When a document has a current matter, that matter must be unarchived for a new document mutation, including archive/restore; its client's archive state alone does not block that existing matter/document association, consistent with D58. With no current matter, an attached direct client must be unarchived for a mutation. A document without either parent is valid. Reassignment validates and locks both the old and new applicable parents; it cannot be used to escape an archived-parent refusal. Restore the blocking parent first. Reads and historical references remain available. Explicitly changing the direct client still requires an unarchived new choice.

6. **Counts, dates and input.** Page count is an optional nonnegative whole PostgreSQL integer: NULL means unknown, zero is distinct, and no arbitrary business maximum is added. Dates are optional valid calendar dates, date-only, years 0001–9999; no assumed today, expiry or date-order rule. An unfinished or invalid native number/date draft must remain visible and be refused before a request is constructed; it is not an intentional NULL. Intentional clearing remains supported. Apply the same allowlist and typed validation at server and database boundaries; no truncating cast, overflow, rounding or normalization of invalid drafts into emptiness.

7. **Text and change semantics.** Preserve untouched Unicode, whitespace, line endings, multiline content, NULL and empty strings. Omitted fields preserve their current values; explicit NULL deliberately clears a nullable field. Use generous measured technical limits and the established 10,000-code-point changed-text/200,000-byte request defaults unless actual legitimate data needs a documented larger limit. These are transport protections, not business rules. Reject unknown fields, forged provenance/audit/identity, malformed payloads and unrecognized selections. Treat M-Files IDs as optional reference text, not arbitrary executable URLs or a reason to invent a numeric-only format.

8. **Reads.** Arabic-first list, normalized search, independent filters, complete counts, stable pagination and full detail are required. Default archive scope is current; offer archived/all. Search technical and Access IDs through exact normalized-digit comparisons, and appropriate current business/linked/source text through explicitly documented text branches. Provide client, matter, responsible-person and M-Files-present/missing filters, with unrecorded choices where meaningful. Archived parents or missing references do not silently suppress otherwise matching reads. Current/source search hits must not misrepresent an unresolved source value as a resolved relationship.

9. **Archive semantics.** Archive is reversible removal from ordinary operational use. Every existing document starts unarchived. It never deletes a row, destroys paper, moves/removes a file, changes custody/page count, closes a matter or changes report inclusion. Archived documents retain clearly labelled, complete read access but are not editable until restored. Transitions preserve all business fields, links, independent retired states and source evidence; neither transition cascades to another record. Existing report criteria/counts remain unchanged unless separately approved.

10. **Confirmation and atomicity.** Archive/restore require a deliberate confirmation naming the document and explaining retention, with Cancel/Escape/focus behavior. Recheck fresh permission, state, version and any displayed related-state assumptions in the committing transaction. Saving cannot silently archive or restore. Each successful change has attributed, append-only audit/history, a new version and a durable exact-submission result; the aggregate and its history commit or fail together. No hard-delete gateway is permitted.

11. **Concurrency, refusal and retry.** Follow the accepted transactional gateway, private allocation, locking and idempotency patterns without weakening them. A true no-op, refusal, stale version, authorization failure or cancelled form causes no business/history/receipt/counter/sequence change. Exact owned retries are checked against actor, action, subject and payload; after later edits they return the original committed result without replay. Fresh account/session/role checks precede receipt reuse. Changed-payload or cross-module token misuse is refused. Measure any unavoidable database reservation behavior rather than rewinding sequences or claiming false zero-change proof. Lock relevant records in a deterministic order and prove actual overlap for conflicting operations.

## D66 — Fee-letter editing, independent matter relationships and archive/restore

Prepared and adopted with D65 under the same combined implementation mandate. Fee letters are engagement-contract records. This contract does not introduce invoices, payment entry, fee calculation, contract approval, legal validity, termination or billing-label policy. Task 4.7a, Task 4.8, reports and Ubuntu deployment remain separate.

1. **Roles.** All four roles read current and explicitly requested archived fee letters. Administrator and Litigation Assistant create/edit fee-letter metadata and the current relationships specified below. Only Administrator archives/restores. Lawyer and Paralegal remain read-only. Preserve the `feeLetters` permission matrix; operations changing a matter's current reference additionally enforce the existing matter-update permission. Apply D65's fresh authorization, audited atomicity, no-op, refusal, concurrency and exact-retry requirements to every entry point.

2. **Identities and editable fields.** PostgreSQL ID is the technical identity; Access `contract_id` is a distinct, preserved import identity. Never renumber imported contracts or treat these IDs as interchangeable. Native rows receive a new technical ID and NULL Access identity/provenance; do not fabricate an Access contract number. Maintain current client, optional M-Files reference, contract type, date, details and structure. New fee letters require an explicitly selected unarchived client. Existing incomplete records remain editable without forced cleanup; deliberate nullable client clearing/replacement on an existing record is attributed and distinct from the imported typed name. Imported `client_name` is original typed evidence, not the authoritative current client; sparse Access `status` remains preserved, labelled source information, not a new workflow or editable status engine.

3. **Source and M-Files.** Preserve every original fee-letter field and all 331 source identities in an immutable boundary. A current M-Files edit/clear never changes `legacy_mfiles_id_raw` or retargets an existing matter/invoice reference. M-Files reference is optional text with no invented uniqueness or numeric-only requirement. Display technical ID, Access contract ID, current M-Files reference and original source reference with distinct labels. Use D65's date/text/input semantics, including invalid native drafts versus intentional clearing.

4. **Two independent relationships.** The source fee-letter covered-matters list and the source matter-side single fee-letter reference are distinct facts. Seed their current states separately from exactly the 231 reviewed forward links and 393 reviewed reverse links. Preserve original link rows, original order/ordinals, source identities/payloads and the 57 plus 19 quarantines unchanged and independently verifiable. Never merge the populations, infer that one is the inverse of the other, seed from quarantined text, or change one automatically when editing the other. Detail and relevant matter views clearly distinguish both current relationships and their original-source evidence.

5. **Editable covered matters.** Include complete add, remove and restore/re-add behavior for a fee letter's current covered-matter memberships, preserving current order where provided. Removing a membership retires retained current state rather than deleting evidence. Existing duplicate imported occurrences keep their distinct identities/order; do not silently deduplicate them. New duplicate selections are refused. Restoring a retired membership is explicit and must not silently revive another retired occurrence. Archiving/restoring the fee letter does not retire or reactivate memberships.

6. **Editable matter-side reference.** Include a distinct current-reference control for a matter's single fee letter, with explicit set, clear and replace actions; it is not the covered-matters checklist. A matter has at most one current reference. Replacing an existing reference requires an explicit choice that names the old and new fee letters; do not steal it through an ordinary 'add' operation or bulk synchronization. Preserve the source `matters.fee_letter_ref`, original reverse-link row and resolution evidence; current changes belong to a separately auditable retained relationship/history boundary. Update necessary existing matter reads/counts to represent current associations accurately while keeping original references separately labelled. The protected raw matter field remains excluded from ordinary matter mutations. Do not change invoices or financial relationships.

7. **Reference resolution.** Prefer labelled selections of exact technical IDs. Whenever a free-text fee-letter reference is accepted or resolved, preserve the established contract-ID/M-Files two-space rule: accept only an unambiguous match in exactly one namespace; reject zero, multiple or both-namespace matches rather than picking the first. Existing historical collisions remain preserved; they are not automatically repaired. Resolve and recheck under the committing lock so a concurrent metadata change cannot retarget a submitted reference. Original import reconciliation uses the immutable historical values, not today's editable M-Files field. Test the actual ambiguity paths independently, including both digit forms where numeric identifiers are normalized.

8. **Parent and related-record eligibility.** A fee letter's current client must be unarchived for a new mutation; existing NULL client remains a supported historical/current state. Validate and lock both old and new client associations during reassignment. All reads remain available under archived clients. New covered-matter choices and changes to a matter-side reference require an unarchived matter; the matter's archived client alone does not block its existing association under D58. Unchanged references to archived matters may remain on a fee letter and must not block unrelated metadata edits. A relationship mutation also requires all affected fee letters to be unarchived and to satisfy their client prerequisite; restore a blocking record first. Lifecycle transitions do not require removing archived-matter links. Lock/version the affected matter, fee letters and parents so competing replacements or confirmation changes cannot silently win.

9. **Reads and navigation.** Provide full Arabic RTL list/search/filter/detail, with current/archive/all scope, client, covered-matter, referencing-matter and M-Files-present/missing filters, and explicit unrecorded options where meaningful. Distinguish the two matter filters. Search all maintained descriptive fields, properly labelled client/source references, matter references and both fee-reference namespaces; technical/Access IDs use exact normalized-digit comparison. Never count joined membership rows as fee-letter results. Detail presents full multiline contract text, both current relationship groups, original evidence and existing invoice links under existing read permissions. Invoice data remains read-only; no billing editor or new totals policy.
