...
BEGIN
  ...
  PERFORM public.audit_write_event('relationship_added','succeeded','public','lookup_client_branch',
  ...
  RETURN NEW;
END $$;
CREATE TRIGGER client_branch_compatibility_audit AFTER INSERT ON _migration.client_branch_compatibility
...
DECLARE t text;
BEGIN
  FOREACH t IN ARRAY ARRAY['client_branch_compatibility','high_impact_application','high_impact_resolution'] LOOP
...
REVOKE ALL ON FUNCTION _migration.refuse_high_impact_evidence_change(),
  ...
  _migration.check_high_impact_completeness() FROM PUBLIC,litigation_runtime;
...
SELECT DISTINCT client_id,branch_id,'D19-existing-association' FROM public.matters WHERE branch_id IS NOT NULL;
COMMIT;
